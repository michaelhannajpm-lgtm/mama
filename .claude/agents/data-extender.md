---
name: data-extender
description: Use when adding new entries to the mock data constants in src/App.jsx — SAMPLE_MOMS, PLACES, SUGGESTED_EVENTS, NEIGHBORHOODS, etc. Preserves the exact shape of existing entries so downstream components don't break.
...
1. **Read the existing constant first.** Find it in `src/App.jsx` (e.g. `const SAMPLE_MOMS = [...]`).
...
5. **Sanity-check** by greping for any component that destructures unfamiliar fields.
