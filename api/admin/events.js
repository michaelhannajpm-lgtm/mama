...
export default async function handler(req, res) {
  ...
  try {
    const select = '*,places:places!events_place_id_fkey(id,name,area,visible,lat,lng,hero_photo),event_categories(category_id)';
    const rows = await fetchAllRows(creds, 'events', `select=${select}&order=created_at.desc`);
